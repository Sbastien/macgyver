...
# Experimental
#
# Kept just in case, or tried once and never removed. This section exists so
# the rest of the file stays honest: anything here is a candidate for deletion
# at the next review rather than something to carry forever.

# Document Toolchain
# @icon:puzzle
# All six are standalone: `brew uses --installed` reports nothing depending on
# any of them, so they were installed deliberately rather than pulled in.
